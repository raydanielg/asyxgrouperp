@php
$title = 'Revenues';
$description = 'Track company revenue streams.';
@endphp
...
<div class="grid grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl border p-4">
        ...
        <p class="text-2xl font-bold text-gray-900 mt-1">{{ $money($monthRevenues ?? 0) }}</p>
    </div>
    <div class="bg-white rounded-xl border p-4">
        <p class="text-[10px] font-medium text-gray-500 uppercase">Records</p>
        <p class="text-2xl font-bold text-gray-900 mt-1">{{ $revenues->total() ?? 0 }}</p>
    </div>
</div>

@include('roles.finance-officer.pages._actions', ['module' => 'revenues'])
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/50">
                    <td class="px-4 py-3 text-xs text-gray-600">{{ $revenue->revenue_date?->format('M d, Y') }}</td>
                    <td class="px-4 py-3 text-xs text-gray-600">{{ $revenue->category ?? '-' }}</td>
                    <td class="px-4 py-3 text-xs text-gray-900">{{ $revenue->description ?? '-' }}</td>
                    <td class="px-4 py-3 text-xs text-gray-900 text-right">{{ $money($revenue->amount) }}</td>
                    <td class="px-4 py-3 text-xs text-gray-600">{{ $revenue->bankAccount?->account_name ?? '-' }}</td>
                </tr>
                @empty
                <tr><td colspan="5" class="px-4 py-8 text-center text-xs text-gray-400">No revenues found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div class="px-4 py-3 border-t flex items-center justify-between">
        <span class="text-xs text-gray-500">Total records: {{ $revenues->total() ?? 0 }}</span>
        <div class="flex items-center gap-2">{{ $revenues->links() }}</div>
    </div>
</div>
@endsection