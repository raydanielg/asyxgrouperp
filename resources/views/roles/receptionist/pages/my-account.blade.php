...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-1">
        <div class="bg-white rounded-xl border p-6 text-center">
            ...
            <p class="text-xs text-gray-400 mt-4">Member since {{ $user['created_at'] ? \Carbon\Carbon::parse($user['created_at'])->format('M d, Y') : '-' }}</p>
        </div>
    </div>
...
function saveProfile(e) {
    ...
    fetch('{{ route('reception.my-account.update') }}', {
        method: 'PUT',
        headers: {
...
function changePassword(e) {
    ...
    fetch('{{ route('reception.my-account.password') }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
