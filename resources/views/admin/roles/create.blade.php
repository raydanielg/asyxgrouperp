...
<form method="POST" action="{{ route('admin.roles.store') }}" class="space-y-4">
    ...
    <div class="bg-white rounded-xl border p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="name" required value="{{ old('name') }}" placeholder="e.g. editor, accountant" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                @error('name')<p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Label *</label>
                <input name="label" required value="{{ old('label') }}" placeholder="e.g. Editor, Accountant" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
    ...
    <div class="flex gap-2">
        ...
        <button type="submit" class="px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700">Create Role</button>
    </div>
</form>
@push('scripts')
<script>
