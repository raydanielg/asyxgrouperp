        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              <h3 class="text-sm font-bold text-gray-900">Role Information</h3>
              @if(!$role->editable)
              <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-50 text-amber-700 border border-amber-100">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                System Role
              </span>
              @else
              <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-sky-50 text-sky-700 border border-sky-100">Custom Role</span>
              @endif
            </div>
            <p class="text-[11px] text-gray-500">Editing role: {{ $role->name }}</p>
          </div>
        </div>
      </div>
      <div class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div class="relative">
              ...
              <input name="name" required value="{{ old('name', $role->name) }}" {{ $role->editable ? '' : 'disabled' }} class="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all {{ $role->editable ? '' : 'bg-gray-50 text-gray-500' }}">
            </div>
            @if(!$role->editable)<p class="text-[10px] text-amber-600 mt-1">System role name is protected.</p>@endif
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input name="label" required value="{{ old('label', $role->label) }}" class="w-full pl-9 pr-3 py-2.5 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all">
            </div>
      ...
      <div class="px-6 py-4 bg-gradient-to-r from-emerald-50 to-white border-b border-gray-100">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div><h3 class="text-sm font-bold text-gray-900">Permissions <span class="text-[10px] font-medium text-gray-400">({{ count($rolePermissions) }} selected)</span></h3><p class="text-[11px] text-gray-500">Modify the permissions for this role</p></div>
          </div>
          <div class="flex items-center gap-2">
      ...
      <div class="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
        ...
        <div class="border border-gray-100 rounded-xl overflow-hidden">
          <div class="flex items-center gap-3 px-5 py-3 bg-gray-50/50 border-b border-gray-100">
            <input type="checkbox" id="module-{{ $moduleSlug }}" class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 module-checkbox w-4 h-4" onchange="toggleModule(this, '{{ $moduleSlug }}')" {{ $allChecked ? 'checked' : '' }}>
            <label for="module-{{ $moduleSlug }}" class="text-xs font-bold text-gray-900 cursor-pointer flex-1">{{ $module }}</label>
            <span class="text-[10px] text-gray-400 font-medium">{{ $checkedCount }}/{{ count($modulePerms) }} selected</span>
          </div>
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1.5 p-4">
            ...
            <label class="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-emerald-50/30 transition-colors cursor-pointer">
              <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" id="perm-{{ $perm->id }}" class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 perm-checkbox module-{{ $moduleSlug }} w-4 h-4" {{ in_array($perm->name, $rolePermissions) ? 'checked' : '' }}>
