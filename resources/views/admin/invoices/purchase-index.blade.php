            ...
            <tbody>
                ...
                <tr class="border-t border-gray-100 hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-5 py-3 flex items-center gap-2">
                        ...
                        <form method="POST" action="{{ route('admin.purchase-invoices.post', $invoice) }}" class="inline"><@csrf><button class="text-emerald-600 hover:text-emerald-700 text-xs">Post</button></form>
                        @endif
...
<div id="createModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onclick="if(event.target===this)this.classList.add('hidden')">
    <div class="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
        <h3 class="text-lg font-bold text-gray-900 mb-4">New Purchase Invoice</h3>
        <form method="POST" action="{{ route('admin.purchase-invoices.store') }}" class="space-y-3">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-3">
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Vendor *</label><select name="vendor_id" required class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none"><option value="">Select...</option>@foreach($vendors as $v)<option value="{{ $v->id }}">{{ $v->name }}</option>@endforeach</select></div>
