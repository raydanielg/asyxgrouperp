        ...
        <tr class="border-t border-gray-100 hover:bg-gray-50/50">
            ...
            <td class="px-5 py-3">
                <div class="flex items-center gap-1.5">
                    <a href="{{ route('admin.delivery-notes.show', $d) }}" title="View" class="w-8 h-8 rounded-lg bg-sky-50 hover:bg-sky-100 text-sky-600 flex items-center justify-center transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    </a>
                    <form id="del-dn-{{ $d->id }}" method="POST" action="{{ route('admin.delivery-notes.destroy', $d) }}" class="inline">@csrf @method('DELETE')</form>
                    <button onclick="confirmDelete('del-dn-{{ $d->id }}')" title="Delete" class="w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 flex items-center justify-center transition-colors">
