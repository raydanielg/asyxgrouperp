...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    <div class="bg-white rounded-xl border p-6">
        ...
        @if($tender->description)<div class="mt-3 pt-3 border-t"><p class="text-[10px] text-gray-400 uppercase mb-1">Description</p><p class="text-xs text-gray-600">{{ $tender->description }}</p></div>@endif
        @if($tender->requirements)<div class="mt-3"><p class="text-[10px] text-gray-400 uppercase mb-1">Requirements</p><p class="text-xs text-gray-600">{{ $tender->requirements }}</p></div>@endif
    ...
    <div class="lg:col-span-2 space-y-4">
        ...
        <div class="bg-white rounded-xl border p-6">
            ...
            @foreach($tender->lead->deals as $deal)<div class="flex items-center justify-between text-xs py-1"><span class="text-gray-700">{{ $deal->title }}</span><span class="text-gray-500">TZS {{ number_format($deal->value) }}</span></div>@endforeach
            </div>
            @endif
        </div>
        @endif
