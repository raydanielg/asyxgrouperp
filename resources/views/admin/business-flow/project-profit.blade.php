...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    ...
    <div class="bg-white rounded-xl border p-6">
        ...
        <div class="mt-4 pt-4 border-t">
            ...
            @foreach($project->budgets as $b)<div class="flex justify-between text-xs py-1"><span class="text-gray-500">{{ $b->budget_number }}</span><span class="text-gray-700">TZS {{ number_format($b->total_budget) }} <span class="inline-flex px-1.5 py-0.5 rounded-full text-[9px] @if($b->status==='approved')bg-emerald-50 text-emerald-700@elseif($b->status==='rejected')bg-red-50 text-red-700@else bg-amber-50 text-amber-700@endif">{{ ucfirst($b->status) }}</span></span></div>@endforeach
        </div>
        @endif
    </div>
