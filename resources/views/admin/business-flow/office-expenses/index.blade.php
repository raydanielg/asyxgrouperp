...
<div class="bg-white rounded-xl border overflow-hidden">
    <div class="overflow-x-auto"><table class="w-full text-sm">
        ...
        <tr class="border-t border-gray-100 hover:bg-gray-50/50">
            ...
            <td class="px-5 py-3"><span class="inline-flex px-2 py-0.5 rounded-full text-[10px] {{ ($e->status==='approved') ? 'bg-emerald-50 text-emerald-700' : (($e->status==='rejected') ? 'bg-red-50 text-red-700' : (($e->status==='disbursed') ? 'bg-sky-50 text-sky-700' : 'bg-amber-50 text-amber-700')) }}">{{ ucfirst($e->status) }}</span></td>
...
<div id="expenseModal" class="hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl max-w-xl w-full">
        ...
        @endforeach
                ...
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Category</label><select name="category" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 outline-none">
        @foreach(['transport'=>'Transport','supplies'=>'Supplies','meals'=>'Meals','utilities'=>'Utilities','misc'=>'Miscellaneous'] as $k=>$v)
