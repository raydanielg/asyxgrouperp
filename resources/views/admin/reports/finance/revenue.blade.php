...
@section('pageSubtitle', 'All revenue entries by period')
...
<div class="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
    ...
    <div class="bg-gradient-to-br from-sky-500 to-sky-600 rounded-xl border border-sky-400 p-4 text-white relative overflow-hidden">
        <div class="absolute top-0 right-0 w-16 h-16 bg-white/10 rounded-full -mr-8 -mt-8"></div>
        <div class="relative z-10">
            <span class="text-[10px] font-medium text-sky-100">This Month</span>
    ...
    <div class="bg-gradient-to-br from-violet-500 to-violet-600 rounded-xl border border-violet-400 p-4 text-white relative overflow-hidden">
        <div class="absolute top-0 right-0 w-16 h-16 bg-white/10 rounded-full -mr-8 -mt-8"></div>
        <div class="relative z-10">
            <span class="text-[10px] font-medium text-violet-100">Avg / Month</span>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-3">
            ...
            <div>
                <div class="flex items-center justify-between mb-1">
                    ...
                    <span class="text-xs font-bold text-emerald-600">{{ $money($m->total) }}</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-2">
                    <div class="bg-gradient-to-r from-emerald-400 to-emerald-600 h-2 rounded-full" style="width: {{ ($byMonth->max('total') > 0 ? ($m->total / $byMonth->max('total')) * 100 : 0) }}%"></div>
                </div>
            </div>
            @endforeach
            @if($byMonth->isEmpty())
    ...
    <div class="lg:col-span-2 bg-white rounded-xl border overflow-hidden">
        <div class="px-5 py-4 border-b">
            <h3 class="text-sm font-semibold text-gray-900">Revenue Entries</h3>
        </div>
