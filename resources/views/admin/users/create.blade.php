...
<form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4">
    ...
    <div class="bg-white rounded-xl border p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="name" required value="{{ old('name') }}" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                @error('name')<p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Email *</label>
                <input name="email" type="email" required value="{{ old('email') }}" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                @error('email')<p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Phone</label>
                <input name="phone" value="{{ old('phone') }}" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
            </div>
            <div>
            ...
            <div>
                ...
                <input name="password" type="password" required minlength="6" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
            ...
            <div>
                ...
                <input name="password_confirmation" type="password" required minlength="6" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
            </div>
        </div>
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_enable_login" value="1" checked class="rounded border-gray-300 text-emerald-600">
    ...
    <div class="bg-white rounded-xl border p-6 space-y-4">
        ...
        <div class="flex items-center gap-2 p-3 border rounded-lg hover:bg-emerald-50/30 transition-colors">
                <input type="checkbox" name="assigned_roles[]" value="{{ $role->id }}" id="role-{{ $role->id }}" class="rounded border-gray-300 text-emerald-600" {{ in_array($role->id, old('assigned_roles', [])) ? 'checked' : '' }}>
    ...
    <div class="flex gap-2">
        ...
        <button type="submit" class="px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700">Create User</button>
    </div>
</form>
@endsection
