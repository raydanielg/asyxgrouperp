...
@section('title', 'SGR Action Points Reports - ' . config('app.name'))
@section('page_title', 'SGR Action Points Reports')
@section('content')
@if(session('success'))
...
<div class="mb-4 flex items-center justify-between">
    <a href="{{ route('admin.sgr.action-points.import') }}" class="text-xs text-gray-500 hover:text-amber-600">&larr; Back to Import</a>
    <div class="flex gap-2">
        <a href="{{ route('admin.sgr.download-template') }}" class="px-3 py-1.5 bg-amber-600 text-white text-xs font-medium rounded-lg hover:bg-amber-700">Download Template</a>
        <a href="{{ route('admin.sgr.index') }}" class="text-xs text-gray-500 hover:text-amber-600">Dashboard</a>
    </div>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl border p-4 text-center">
        <p class="text-2xl font-bold text-red-600">{{ number_format($summary['overdue']) }}</p>
...
<div class="bg-white rounded-xl border p-5 mb-6">
    ...
    <form method="GET" action="{{ route('admin.sgr.action-points.reports') }}" class="grid grid-cols-1 md:grid-cols-5 gap-3">
        <div>
        ...
        <div>
            ...
            <input type="text" name="responsible" value="{{ $responsible }}" placeholder="Search name..." class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 outline-none">
        </div>
        <div>
            <label class="block text-[10px] text-gray-500 mb-1">Status</label>
            <input type="text" name="status" value="{{ $status }}" placeholder="e.g. ACHIEVED" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 outline-none">
        </div>
        <div>
        ...
        <div>
            ...
            <select name="approval_status" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 outline-none">
        ...
        <div class="md:col-span-5 flex gap-2">
            ...
            <a href="{{ route('admin.sgr.action-points.reports') }}" class="px-4 py-2 border border-gray-200 text-gray-600 text-sm rounded-lg hover:bg-gray-50">Clear</a>
        </div>
    </form>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-4 py-8 text-center text-gray-400 text-xs">No action points found. <a href="{{ route('admin.sgr.action-points.import') }}" class="text-amber-600 hover:text-amber-700">Import now</a></td>
                </tr>
                @endforelse
            </tbody>
        </table>
