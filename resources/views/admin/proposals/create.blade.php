...
@section('title', 'Create Proposal - ' . config('app.name'))
@section('page_title', 'Create Sales Proposal')
@section('content')
<div class="max-w-4xl">
    <form method="POST" action="{{ route('admin.sales-proposals.store') }}" class="space-y-6">
        ...
        <div class="bg-white rounded-xl border p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div><label class="block text-xs font-medium text-gray-600 mb-1">Proposal Date *</label><input name="proposal_date" type="date" value="{{ date('Y-m-d') }}" required class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none"></div>
        ...
        <div class="bg-white rounded-xl border p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-sm font-semibold text-gray-900">Proposal Items</h3>
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700">Create Proposal</button>
        </div>
    </form>
</div>
<script>
