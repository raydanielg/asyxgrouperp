...
@section('title', 'Create Application - ' . config('app.name'))
@section('page_title', 'New Application')
@section('content')
<div class="bg-white rounded-xl border p-6 max-w-2xl">
  <form method="POST" action="{{ route('admin.applications.store') }}" enctype="multipart/form-data" class="space-y-4">@csrf
    <div>
      <label class="block text-xs font-medium text-gray-600 mb-1">Job Posting *</label>
      <select name="job_posting_id" required class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm">
        @foreach($jobs as $j)
    ...
    <div class="pt-2 flex gap-3">
      <a href="{{ route('admin.applications.index') }}" class="px-4 py-2 border border-gray-200 text-gray-600 text-sm rounded-lg hover:bg-gray-50">Cancel</a>
      <button class="px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700">Create</button>
    </div>
  </form>
</div>
@endsection
