...
<form method="POST" action="{{ route('admin.employees.store') }}" class="space-y-4">
    ...
    <div class="bg-white rounded-xl border p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><label class="block text-xs font-medium text-gray-600 mb-1">Gender</label><select name="gender" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none"><option value="">Select...</option>@foreach(['male'=>'Male','female'=>'Female','other'=>'Other'] as $k=>$v)<option value="{{ $k }}" @selected(old('gender')===$k)>{{ $v }}</option>@endforeach</select></div>
            ...
            <div><label class="block text-xs font-medium text-gray-600 mb-1">Nationality</label><select name="nationality" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none"><option value="">Select...</option>@foreach(['Kenyan','Tanzanian','Ugandan','Nigerian','South African','American','British','Indian','Other'] as $nat)<option value="{{ $nat }}" @selected(old('nationality')===$nat)>{{ $nat }}</option>@endforeach</select></div>
            <div><label class="block text-xs font-medium text-gray-600 mb-1">Marital Status</label><select name="marital_status" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none"><option value="">Select...</option>@foreach(['single'=>'Single','married'=>'Married','divorced'=>'Divorced','widowed'=>'Widowed'] as $k=>$v)<option value="{{ $k }}" @selected(old('marital_status')===$k)>{{ $v }}</option>@endforeach</select></div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="department" required class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                    ...
                    @foreach($departments as $dept)<option value="{{ $dept }}" @selected(old('department')===$dept)>{{ $dept }}</option>@endforeach
                </select>
            </div>
            <div>
                ...
                <select name="designation" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                    ...
                    @foreach($designations as $des)<option value="{{ $des }}" @selected(old('designation')===$des)>{{ $des }}</option>@endforeach
                </select>
            </div>
            <div>
                ...
                <select name="manager_id" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                    ...
                    @foreach($managers as $m)<option value="{{ $m->id }}" @selected(old('manager_id')==$m->id)>{{ $m->full_name }} ({{ $m->designation ?? 'N/A' }})</option>@endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="employment_type" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                    ...
                    @foreach($employmentTypes as $et)<option value="{{ $et }}" @selected(old('employment_type')===$et)>{{ $et }}</option>@endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="status" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none">
                    @foreach(['active'=>'Active','inactive'=>'Inactive','on_leave'=>'On Leave','terminated'=>'Terminated'] as $k=>$v)<option value="{{ $k }}" @selected(old('status', 'active')===$k)>{{ $v }}</option>@endforeach
                </select>
            </div>
            <div><label class="block text-xs font-medium text-gray-600 mb-1">Shift</label><select name="shift" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none"><option value="">Select...</option>@foreach(['morning'=>'Morning (8AM-4PM)','evening'=>'Evening (4PM-12AM)','night'=>'Night (12AM-8AM)','flexible'=>'Flexible'] as $k=>$v)<option value="{{ $k }}" @selected(old('shift')===$k)>{{ $v }}</option>@endforeach</select></div>
            <div><label class="block text-xs font-medium text-gray-600 mb-1">Work Location</label><select name="work_location" class="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none"><option value="">Select...</option>@foreach(['office'=>'On-site','remote'=>'Remote','hybrid'=>'Hybrid'] as $k=>$v)<option value="{{ $k }}" @selected(old('work_location')===$k)>{{ $v }}</option>@endforeach</select></div>
        </div>
    </div>
