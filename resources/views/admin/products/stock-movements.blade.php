...
<div class="bg-white rounded-xl border overflow-hidden">
    <div class="overflow-x-auto"><table class="w-full text-sm">
        ...
        <tr class="border-t border-gray-100 hover:bg-gray-50/50">
            ...
            <td class="px-5 py-3"><span class="inline-flex px-2 py-0.5 rounded-full text-[10px] font-medium {{ ($m->type=='in') ? 'bg-emerald-50 text-emerald-700' : ($m->type=='out') ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700' }}">{{ ucfirst($m->type) }}</span></td>
            <td class="px-5 py-3 text-xs font-semibold {{ ($m->type=='in') ? 'text-emerald-700' : ($m->type=='out') ? 'text-red-600' : 'text-amber-700' }}">{{ $m->type == 'in' ? '+' : ($m->type == 'out' ? '-' : '') }}{{ $m->quantity }}</td>
