$roles = @(
    @{name='admin-manager'; label='Admin Manager'; color='emerald'; chart='Sales vs Purchases'; actions=@('Users','Roles & Permissions','Reports')},
    @{name='administrator'; label='Administrator'; color='emerald'; chart='Sales vs Purchases'; actions=@('Users','Employees','Settings')},
    @{name='auditor'; label='Auditor'; color='sky'; chart='Revenue vs Expenses'; actions=@('Reports','Sales Invoices','Purchase Invoices')},
    @{name='legal-officer'; label='Legal Officer'; color='violet'; chart='Activity'; actions=@('Contracts','Projects')},
    @{name='receptionist'; label='Receptionist'; color='amber'; chart='New Leads'; actions=@('Leads','Contacts','Tickets')},
    @{name='logistics-officer'; label='Logistics Officer'; color='sky'; chart='Stock Movements'; actions=@('Products','Warehouses','Transfers')},
    @{name='technical-manager'; label='Technical Manager'; color='rose'; chart='Tickets Created'; actions=@('Tickets','Projects','Employees')},
    @{name='technician'; label='Technician'; color='rose'; chart='Tickets Created'; actions=@('My Tickets','Projects')},
    @{name='ict-officer'; label='ICT Officer'; color='sky'; chart='Tickets Created'; actions=@('Tickets','Projects','Assets')},
    @{name='project-manager'; label='Project Manager'; color='emerald'; chart='Project Activity'; actions=@('Projects','Timesheets','Deals')},
    @{name='operations-manager'; label='Operations Manager'; color='emerald'; chart='Project Activity'; actions=@('Products','Projects','Warehouses')},
    @{name='call-center-agent'; label='Call Center Agent'; color='amber'; chart='New Leads'; actions=@('Leads','Contacts','Tickets')},
    @{name='supervisor'; label='Supervisor'; color='violet'; chart='Attendance Trend'; actions=@('Attendance','Leaves','Projects')},
    @{name='ict-engineer'; label='ICT Engineer'; color='rose'; chart='Tickets Created'; actions=@('Tickets','Projects','Settings')}
)

$basePath = "c:\Users\Administrator\Desktop\Djanproject\resources\views\roles"

foreach ($role in $roles) {
    $dir = Join-Path $basePath $role.name
    if (!(Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }

    $view = @"
@extends('layouts.admin')
@section('title', '$($role.label) Dashboard')
@section('page_title', '$($role.label) Dashboard')
@section('content')
...
<div class="bg-gradient-to-r from-emerald-700 to-emerald-900 rounded-xl p-6 mb-6 text-white relative overflow-hidden">
    ...
    <div class="relative z-10 flex items-center justify-between">
        <div><h2 class="text-2xl font-bold">Welcome, {{ auth()->user()->name }}</h2><p class="text-emerald-100 text-sm mt-1">$($role.label) Dashboard</p></div>
    ...
    <div class="bg-white rounded-xl border overflow-hidden">
        ...
        <div class="divide-y divide-gray-100">
            ...
            <div class="px-5 py-3 flex items-center justify-between hover:bg-gray-50/50">
                ...
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium @if(`$att->status === 'present') bg-emerald-50 text-emerald-700 @elseif(`$att->status === 'absent') bg-rose-50 text-rose-700 @else bg-amber-50 text-amber-700 @endif">{{ ucfirst(`$att->status) }}</span>
            </div>
            @endforeach
        </div>
    </div>
...
"@
    ...
    $filePath = Join-Path $dir "dashboard.blade.php"
    Set-Content -Path $filePath -Value $view -NoNewline -Encoding UTF8
